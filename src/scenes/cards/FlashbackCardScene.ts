...
export class FlashbackCardScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.card.options.forEach((opt, i) => {
      ...
      this.add.text(cx - this.optionWidth / 2 + 20, y, optionLabels[i], { fontSize: '18px', fontFamily: FONT.title, color: HEX.green }).setOrigin(0, 0.5);
  ...
  private highlightCorrectOption(): void {
    ...
    this.add
      .text(this.optionCenterX, 645, `Correct answer: ${label}`, {
        fontSize: '14px',
